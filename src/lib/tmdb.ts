// TMDB API client — calls api.themoviedb.org directly (no backend required).
...
export async function tmdb<T = any>(path: string, params: Record<string, string | number> = {}): Promise<T> {
  ...
  search.set("api_key", TMDB_KEY);
  if (!search.has("language")) search.set("language", "en-US");
  const res = await fetch(`${TMDB_BASE}${pathname}?${search.toString()}`, {
